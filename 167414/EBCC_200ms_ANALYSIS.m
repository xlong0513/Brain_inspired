 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 %          RESULT ANALYSIS FOR EBCC SIMULATION ISI = 200 ms               %
 %                           -------------------                           %
...
fspk=zeros((ending-starting)*1000,MFNum);
for i=1:MFNum
    for j=1:size(fspk,1)
...
fspk=zeros((ending-starting)*1000,IONum);
for i=1:IONum
    for j=1:size(fspk,1)
...
fspk=zeros((ending-starting)*1000,DCNNum);
for i=1:DCNNum
    for j=1:size(fspk,1)
...
for startingzoom = starting %SET THE START OF THE TRIAL
...
j=1;
for i=1:seiv-1
    MatrixDCN(j,1:500)=DCN_save(j,:);
    plot3(10*(j-1)*ones(size(10:500)),10:500,MatrixDCN(j,10:500),'k')
    hold on
    j=j+1;
end
box off
...
j=1;
for i=1:seiv-1
    MatrixPC(j,1:500)=PC_save(j,:);
    plot3(10*(j-1)*ones(size(1:500)),1:500,MatrixPC(j,1:500),'g')
    hold on
    j=j+1;
end
box off
...
count=1;
for i=1:nrep
    CR_ordered2=CR_ordered(find(CR_ordered<=(i/2)));
    CRS=CR_ordered2(find(CR_ordered2>(i/2-5)))
...
total = 0;
for i=1:size(FinalWeights10,1)
    total=total+FinalWeights10(i,1);
end
